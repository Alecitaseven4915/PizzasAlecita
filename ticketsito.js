document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('ticket-cvv').textContent = datos.cvv || '***';

  // Saldo aleatorio (mismo que en el formulario)
  const saldo = (Math.random() * (5000 - 1000) + 1000).toFixed(2);
...
function generatePDF() {
  ...
  productos.forEach((producto, index) => {
    doc.text(`${index + 1}. ${producto.nombre} - $${producto.precio.toFixed(2)}`, 10, yPosition);
    yPosition += 10;
    total += producto.precio;
  });
